...
public class Board extends JPanel {
	...
	private int cols = -1;
	/**
	 * 2d matrix sent by user
	 */
	private int[][] matrix2d;
	/**
	 * type of draw requested by user
	 */
	private char type2d;
	/**
	...
	private char[] type3d;
	/**
	 * selector of the type of draw. false = non-layered matrix. true = layered
	 * matrix
	 */
	...
	protected void paintComponent(Graphics g) {
		...
		if (init) { // nothing will be shown until the first draw call
			...
			if (!layered) {
				...
				initSquares(matrix2d.length, matrix2d[0].length);
				for (int i = 0; i < matrix2d.length; i++) {
					for (int j = 0; j < matrix2d[0].length; j++) {
						squares[i][j].draw(g2, matrix2d[i][j], type2d, colors, actborder, borderColor, backgroundColor,
								text, colortext, sprites, actimgbackground, actfreedraw, freedrawx, freedrawy, font);
					}
				}
			} else {
				// several matrixes
...
class Square {
	...
	public void draw(Graphics2D g2, int i, char type, int[] colors, boolean actborder, int borderColor,
		...
		if (type == 't') {
			Color inside = new Color(colortext[value]);
			g2.setPaint(inside);
			g2.setFont(font);
